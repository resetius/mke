...
sub insert_data
{
	...
	my $s = "INSERT INTO $tabname(step,t,wt,nr1,mn1,mx1,v1) VALUES(?,?,?,?,?,?,?)";
...
my $cmd = "./bin/test_baroclin -t 1 --task kornev1 -f tt.txt -v 10 --time 100 2>&1 | ";
open(PIPE, $cmd);
print "run $cmd\n";
		...
		if ($_ =~ m/^# end.*\n/) {
			if (not $step_interval or $total % $step_interval == 0) {
				...
				if ($u == 0) {
					...
					print STDERR "update $step, $t, $wt, $nr, $mn, $mx \n";
					update_data($uniq_table_name, $step, $t, $wt, $nr, $mn, $mx, $cur);
				}
